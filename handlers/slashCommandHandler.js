...
const dirSetup = [{
    "Folder": "Info",
    "CmdName": "info",
    "CmdDescription": "Get information about any command!!"
}, {
    "Folder": "Admin",
    "CmdName": "admin",
    "CmdDescription": "Configuration commands to change settings of your server"
}, {
    "Folder": "AI-Chat",
    "CmdName": "ai-chat",
    "CmdDescription": "Change settings of ai-chat bot in your server"
}, {
    "Folder": "Moderation",
    "CmdName": "moderation",
    "CmdDescription": "Moderate your server using this commands"
}, {
    "Folder": "Fun",
    "CmdName": "fun",
    "CmdDescription": "Entertain yourself with this commands!"
}, {
    "Folder": "Utility",
    "CmdName": "utility",
    "CmdDescription": "Extra useful commands"
}];
module.exports = (client) => {
    try {
                                    ...
                                    if (pull.options && pull.options.length > 0) {
                                        for (const option of pull.options) {
                                            if (option.User && option.User.name && option.User.description) {
                                                ...
                                                console.log(`A Option is missing the Name or/and the Description of ${pull.name}`.red)
                                            }
                                        }
                                    }
                                    return subcommand;
        ...
        client.on("ready", () => {
            if (config.registarSlashGLobal) {
                client.application.commands.set(allCommands)
                    .then(slashCommandsData => {
                        client.slashCommandsData = slashCommandsData;
                        console.log(`${slashCommandsData.size} slashCommands ${`(With ${slashCommandsData.map(d => d.options).flat().length} Subcommands)`.purple} Loaded for all guilds`.brightYellow);
                    }).catch((e) => {});
            } else {
                client.guilds.cache.map(g => g).forEach(async (guild) => {
                    try {
                        ...
                        guild.commands.set(allCommands)
                            .then(slashCommandsData => {
                                client.slashCommandsData = slashCommandsData;
                                console.log(`${slashCommandsData.size} slashCommands ${`${slashCommandsData.map(d => d.options).flat().length} Subcommands)`.green} Loaded for guilds`.brightGreen);
                            }).catch((e) => {});
                    } catch (e) {
